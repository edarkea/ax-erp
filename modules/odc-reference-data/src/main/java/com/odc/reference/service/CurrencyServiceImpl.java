...
public class CurrencyServiceImpl implements CurrencyService {
  ...
  @Override
  @Transactional(rollbackOn = Exception.class)
  public Currency archive(Currency currency) {
    if (currency == null || currency.getId() == null) {
      throw inconsistency("Currency must be saved before it can be archived.");
    }
  ...
  protected Currency findOtherByCode(String code, boolean archived, Long excludedId) {
    ...
    if (excludedId != null) {
      query = query.filter("self.id != :excludedId").bind("excludedId", excludedId);
    }
